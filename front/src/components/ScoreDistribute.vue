<template>
    <div style="margin: 0 auto;margin-top: 20px;margin-left: 50px">
      <div class="score-detail">
        <el-card v-for="(info, index) in questionList" style="text-align: left;padding: 0;margin: 20px;" @dblclick.native="gotoDetail(index)">
      ...
      <div class="aside-window">
        <span>您已经提问了{{questionNum}}个问题, 有{{totalScore}}个积分正在等你分配哟</span>
      </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
          ...
          loadQuestionList: function(){
            const _this = this
            question.scoreQuestion(this.$store.getters.getUserID, function(scoreDistri){
